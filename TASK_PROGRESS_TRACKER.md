...
| **T-7.1** | Fix Gift Sending `rocketWinners is not defined` block-scoping bug in `sendGiftWithCombo` and deploy live to Firebase Cloud Functions | `functions/index.js` | ✅ **DONE** | Deployed to `hellochat-e8965` via firebase-tools; verified in Node.js syntax & unit tests | Ready for User Check |
| **T-7.2** | Top Floating Overlay Toast (`AppToast.showOverlay`): Render on top of modal bottom sheets & dialogs | `lib/core/widgets/app_toast.dart`, `gift_panel.dart` | ✅ **DONE** | Verified on real device: error and validation alerts display directly over gift sheet | Ready for User Check |
| **T-7.3** | Spin Wheel Network Connectivity: Prevent false "Network Connection Required" barrier on active internet | `lib/features/games/presentation/screens/spin_wheel_screen.dart` | ✅ **DONE** | Tested in `test/offline_betting_prevention_test.dart` (4/4 passed) | Ready for User Check |
| **T-7.4** | Ultra-Fast Room Seat Switching: Remove 4 redundant sequential Firestore reads & run Agora switch concurrently (<150ms) | `lib/services/room_service.dart`, `live_room_screen.dart` | ✅ **DONE** | Live verified on device with 0 layout/stream conflicts | Ready for User Check |
